...
{
	//Utils::ToLog("CANHIT");
	auto *local = I::EntList->GetClientEntity(I::Engine->GetLocalPlayer());
	...
	if (SimulateFireBullet(local, (CBaseCombatWeapon*)I::EntList->GetClientEntityFromHandle((HANDLE)local->GetActiveWeaponHandle()), data))
	{
